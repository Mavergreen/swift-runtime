...
#          usr/local/mavergreen-swift-runtime/LICENSE.txt. <out-dir> persists between builds, so
#          everything an earlier layout left under usr/ (and a stray LICENSE.txt at the payload
#          root) is removed first; <out-dir>/Library belongs to package.sh and is left alone.
set -eu
